...
begin
  if not exists (
    select 1
    from pg_constraint
    where conname = 'spotify_playlist_tracks_cache_language_code_valid'
  ) then
    alter table public.spotify_playlist_tracks_cache
    add constraint spotify_playlist_tracks_cache_language_code_valid
    check (
...
begin
  ...
  select
    ...
    lower(nullif(btrim(track.language_code), ''))
  ...
  set
    ...
    language_code = coalesce(incoming_track.language_code, existing_track.language_code),
